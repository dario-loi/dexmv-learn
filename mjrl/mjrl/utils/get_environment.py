...
def get_environment(env_name=None):
    ...
    if task == "pour":
        env = WaterPouringEnv(
            has_renderer=False, scale=obj_scale, tank_size=(0.15, 0.15, 0.12)
        )
        return DAPGWrapper(env)
    if task == "place":
        env = MugPlaceObjectEnv(
            has_renderer=False, object_scale=obj_scale, mug_scale=1.5, large_force=True
        )
        return DAPGWrapper(env)
