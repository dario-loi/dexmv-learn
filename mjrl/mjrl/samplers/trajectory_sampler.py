...
def sample_paths_parallel(
    ...
    results = _try_multiprocess(
        args_list, num_cpu, max_process_time, max_timeouts, mode
    )
    paths = []
...
def _try_multiprocess(args_list, num_cpu, max_process_time, max_timeouts, mode):
    ...
    if mode == "sample":
        ...
        parallel_runs = [
            pool.apply_async(
                eval_sampler.do_evaluation_rollout_star, args=(args_list[i],)
            )
            for i in range(num_cpu)
        ]
    else:
    ...
    try:
        ...
        return _try_multiprocess(
            args_list, num_cpu, max_process_time, max_timeouts - 1, mode
        )
