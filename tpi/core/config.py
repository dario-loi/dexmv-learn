...
def assert_cfg():
    assert not (_C.BC_INIT or _C.USE_DAPG) or os.path.exists(
        cfg.DEMO_FILE
    ), "Demo file not found: {}".format(cfg.DEMO_FILE)
    assert (_C.SDAPG_OBS_L == 0) or _C.SDAPG_DIST in [
        "h_dtw",
        "mp_dtw",
    ], "Using obs left ind and {} dist not supported".format(_C.SDAPG_DIST)
    assert (_C.SDAPG_OBS_R == 39) or _C.SDAPG_DIST in [
        "h_dtw",
        "mp_dtw",
    ], "Using obs right ind and {} dist not supported".format(_C.SDAPG_DIST)
    assert (
        0.0 <= _C.DENSITY_ONPG_POS_FRAC <= 1.0
    ), "Positives fraction must be in [0, 1]"
    assert (
        not _C.CUSTOM_OBJECT or _C.ENV_NAME == "relocate-v0"
    ), "Using custom objects supported only with relocate"
    assert (
        not _C.CUSTOM_MASS or _C.ENV_NAME == "relocate-v0"
    ), "Using custom mass supported only with relocate"
    assert (
        not _C.CUSTOM_FRICT or _C.ENV_NAME == "relocate-v0"
    ), "Using custom friction supported only with relocate"
    assert (
        not _C.CUSTOM_OBJ_MASS or _C.ENV_NAME == "relocate-v0"
    ), "Using custom object mass supported only with relocate"
    assert (
        not _C.CUSTOM_OBJ_SIZE or _C.ENV_NAME == "relocate-v0"
    ), "Using custom object size supported only with relocate"
    assert (
        not _C.CUSTOM_FINGERS or _C.ENV_NAME == "relocate-v0"
    ), "Using custom fingers supported only with relocate"
    assert (
        not _C.INVDYN_ONPG_ACT_SUBSET or _C.CUSTOM_FINGERS
